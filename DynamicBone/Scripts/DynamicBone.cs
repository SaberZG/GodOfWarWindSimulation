...
public class DynamicBone : MonoBehaviour
{
    ...
    [FormerlySerializedAs("m_UseWindForce")] [Tooltip("Use wind force created from custom global volume wind data.")]
    public float m_WindForce = 1;
    [FormerlySerializedAs("m_UseWindForceDistrib")] public AnimationCurve m_WindForceDistrib = null;
    ...
    void OnValidate()
    {
        ...
        m_WindForce = Mathf.Max(m_WindForce, 0);
    ...
    public void UpdateParameters()
    {
        ...
        for (int i = 0; i < m_Particles.Count; ++i)
        {
            ...
            if (m_BoneTotalLength > 0)
            {
                ...
                if (m_WindForceDistrib != null && m_WindForceDistrib.keys.Length > 0)
                    p.m_WindForce *= m_WindForceDistrib.Evaluate(a);
            }
            ...
            p.m_WindForce = Mathf.Max(p.m_WindForce, 0);
        }
    }
    ...
    void UpdateParticles1(float timeVar)
    {
        ...
        for (int i = 0; i < m_Particles.Count; ++i)
        {
            ...
            if (p.m_ParentIndex >= 0)
            {
                ...
                if (m_WindForce > 0f)
                {
                    Vector3 windForce = WindManager.Instance.GetWindForceByPosAndDeltaTime(p.m_Position);
                    windForce *= p.m_BoneLength / m_BoneTotalLength;
                    force += windForce * p.m_WindForce;
                }
